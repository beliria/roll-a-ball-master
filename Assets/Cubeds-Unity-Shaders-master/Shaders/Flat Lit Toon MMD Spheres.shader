Shader "CubedParadox/Flat Lit Toon MMD Spheres"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(VertexOutput i) : COLOR
			{
				...
				float3 finalColor = emissive + (baseColor * sphereMul + sphereAdd) * lerp(indirectLighting, directLighting, directContribution);
		...
		Pass
		{
			...
			float4 frag(VertexOutput i) : COLOR
			{
				...
				float3 finalColor = (baseColor * sphereMul + sphereAdd) * lerp(0, _LightColor0.rgb, saturate(directContribution + ((1 - _Shadow) * attenuation)));
	...
	FallBack "Diffuse"
	CustomEditor "FlatLitToonInspectorMMDSpheres"
}